...
class HomeScreen extends Component {
    ...
    courselist = async () => {
        var listref = await firebase.database().ref("courses/").on("value", dataSnapshot => {
    ...
    render() {
        return (
            <View style={{ backgroundColor: '#EAF0F1', flex: 1 }}>
                ...
                <TouchableOpacity style={{ position: 'absolute', top: Platform.OS === 'ios' ? hp('4%') : hp('2%'), left: Platform.OS === 'ios' ? hp('2%') : hp('2%') }} onPress={() => this.props.navigation.openDrawer()}>
                    <Icon name='ios-menu' size={40} color='#fff' />
                </TouchableOpacity>
                <Text style={{ color: '#121212', textTransform: 'uppercase', fontWeight: 'bold', padding: 20, letterSpacing: 1, fontSize: 30, justifyContent: 'center', alignSelf: 'center' }}>Courses</Text>
                    ...
                    <View style={{ flex: 1, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around' }}>
                        {this.state.courseslist.map((item, index) => (
                            <TouchableOpacity key={index} onPress={() => this.props.navigation.navigate('DetailScreen', { content: item })}>
                                <GridImage image={{ uri: item.image }} text={item.name} para={item.para} width={width / 3 - 10} />
                            </TouchableOpacity>
                        ))}
                    </View>
                </ScrollView>
